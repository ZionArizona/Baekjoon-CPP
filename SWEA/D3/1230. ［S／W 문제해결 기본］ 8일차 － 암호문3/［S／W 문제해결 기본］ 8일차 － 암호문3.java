...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int test = 1; test <= 10; test++){
            ...
            while(idx < instruct.length){
                ...
                else if(instruct[idx].equals("A")){
                    idx = addition(instruct, idx);
                }
    ...
    static int addition(String[] instruct, int index) {

        int cnt = Integer.parseInt(instruct[index + 1]);

        for(int i= 0; i < cnt; i++){
            list.add(instruct[index+1+i]);
        }
